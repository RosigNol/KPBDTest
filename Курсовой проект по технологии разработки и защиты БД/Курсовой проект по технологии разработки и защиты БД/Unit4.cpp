...
//int kol_vo_buff=0;
String timestart;
struct Question *elem; /* новый элемент */
struct Question *start=NULL; /* начало списка */
...
{
  while (start!=NULL)  //Пока по адресу не пусто
     {
		Question *temp=start->next; //Временная переменная для хранения адреса следующего элемента
		delete start; //Освобождаем адрес обозначающий начало
		start=temp; //Меняем адрес на следующий
	 }
}
//------------------------------------------------------------------------------
void CreateSp (struct Question *i, /* новый элемент */
			   struct Question **start, /* начало списка */
			   struct Question **last) /* конец списка */
{
...
//--------------------------------------------------------------------------
void deletesp(
struct Question *p, /* предыдущий элемент */
	 struct Question *i, /* удаляемый элемент */
	 struct Question **start, /* начало списка */
	 struct Question **last) /* конец списка */
{
  if(p) p->next = i->next;
...
void __fastcall TForm4::sButton1Click(TObject *Sender)
{
   ...
   DataModule1->ADOQuery1->SQL->Text=("exec Добавление_промежуточных_результатов "
   ...
   {
   i++;
   Form4->sLabel5->Caption="Вопрос № "+IntToStr(i)+".";
   //Randomize();
...
if (Form2->sComboBox1->Text=="Программирование на Delphi") {
...
if (Form2->sComboBox1->Text=="Основы Access") {
Graphics::TBitmap *ptBitmap;
   ptBitmap=new Graphics::TBitmap();
   TResourceStream* ptRes =
...
if (Form2->sComboBox1->Text=="Основы С++") {
Graphics::TBitmap *ptBitmap;
   ptBitmap=new Graphics::TBitmap();
   TResourceStream* ptRes =
...
if (Form2->sComboBox1->Text=="Технология разработки и защиты баз данных") {
Graphics::TBitmap *ptBitmap;
   ptBitmap=new Graphics::TBitmap();
   TResourceStream* ptRes =
   ...
   i++;
   struct Question *prev=NULL;
   Form4->sLabel1->Caption="Дисциплина: "+Form2->sComboBox1->Text;
   Form4->sLabel2->Caption="Тема: "+Form2->sComboBox2->Text;
   Form4->sLabel5->Caption="Вопрос № "+IntToStr(i)+".";
   Form4->sButton1->Caption="Перейти к следующему вопросу.";
   DataModule1->ADOQuery1->Close();
   DataModule1->ADOQuery1->SQL->Clear();
